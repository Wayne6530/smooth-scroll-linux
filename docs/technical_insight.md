...
Releasing the activation button while either enabled axis has non-zero speed latches the mode, so output continues hands-free. Releasing while every enabled axis is paused exits instead; a sufficiently short release is replayed as a native click. This decision depends only on the current displacement, so moving out of and back into the dead zone is equivalent to a short click. Pointer movement continues changing the virtual offset without moving the visible pointer.

`auto_scroll_wheel_action` controls whether a physical wheel event is ignored while Auto Scroll is latched or consumed to exit without producing scroll. By default, `auto_scroll_exit_button_mode` allows an ordinary button press to become the held-exit control; it can instead restrict that control to the activation button. The initiating press and matching release are consumed; while it is held, other buttons pass through except for Free Spin, which retains its special behavior. The option changes only how a press is captured: an unclaimed release or repeat exits Auto Scroll immediately and passes through in either mode, preserving native pairs for buttons held before Auto Scroll was latched.

Each Auto Scroll axis uses its own fractional output accumulator so low speeds survive integer event quantization. Its virtual offset saturates at the smallest integral displacement that reaches the configured maximum speed, discarding further movement in the same direction so the reported offset remains directly related to speed. In omnidirectional mode, the daemon emits the horizontal and vertical high-resolution wheel events in one input report. It also broadens the user-facing "scrolling" predicate used by Drag View and Free Spin eligibility, while the physics engine continues using the narrower ordinary-inertia predicate for decay, reverse braking, and mouse-movement braking.
