...
const I18n = (() => {
  ...
  function t(key) {
    const translations = {
      ...
      'feature.smooth.desc': { en: 'Raw wheel input arrives in hard 120-unit steps. Smooth Scroll keeps your intent, then emits smaller, timed steps that feel continuous.', zh: '原始滚轮输入是一段段 120 单位的跳变。Smooth Scroll 保留你的操作意图，再输出更细、更有节奏的滚动事件。' },
      ...
      'feature.adaptive.desc': { en: 'The daemon measures how quickly wheel events arrive. Gentle scrolling stays controlled; quick bursts build speed so long pages stop feeling heavy.', zh: '守护进程会根据滚轮事件的间隔判断速度。轻轻滚动时更可控，快速连滚时会建立速度，让长页面不再沉重。' },
      ...
      'feature.more.config.desc': { en: 'Edit 24 parameters in the web configurator, preview the motion, then export TOML.', zh: '在 Web 配置器中编辑 24 个参数，预览滚动效果，然后导出 TOML。' },
      ...
      'feature.more.light.desc': { en: 'Single-threaded C++, a tiny binary, low CPU usage, and no runtime dependency stack.', zh: 'C++ 单线程实现，二进制体积小，CPU 占用低，无复杂运行时依赖。' },
